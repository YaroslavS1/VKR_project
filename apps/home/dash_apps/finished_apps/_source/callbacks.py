...
# Sales mapping
sales_filepath = '/home/y_sukhorukov/VKR/VKR_PROJECT/datasource.xlsx'
...
# from apps.home.views import _crm
# sales_import = pd.merge(a1.append(a2), _crm.slice_by_day, on=['date', 'source', 'name'], how="outer")
from .data_helper import sales_import

# sales_import = a1.append(a2)
...
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
    ...
    # Aggregate df
    val_cols = [sales_fields['revenues'], sales_fields['rev target']]
    df = df1.groupby('date')[val_cols].agg('sum')
    ...
    hovertemplate_xy = (
            "%{x}<br>" +
            "<i>Потречаено на рекламу</i>: %{y}" +
            "<extra></extra>")  # Remove trace info
    data = go.Bar(
        x=df['date'],
        y=df[sales_fields['rev target']],
    ...
    hovertemplate_xy2 = (
            "%{x}<br>" +
            "<i>Получено прибыли</i>: %{y}" +
            "<extra></extra>")  # Remove trace info
    fig.add_trace(
        go.Scatter(
            x=df['date'],
            y=df[sales_fields['revenues']],
    ...
    fig.update_layout(
        title={'text': "Расходы на рекламу vs Прибыль с рекламы"},
        xaxis={
            'title': "Время",
            'tickformat': "%d-%m-%y"
